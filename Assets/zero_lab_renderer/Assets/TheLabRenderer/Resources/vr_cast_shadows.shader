...
#define _Color_arr InstanceProperties
			...
			float4 MainPs(VertexOutput i) : SV_Target
			{
				...
				float2  newUV = TRANSFORM_TEX(i.uv0, _MainTex);
				 half alpha = tex2D(_MainTex, newUV).a * UNITY_ACCESS_INSTANCED_PROP(_Color_arr, _Color).a;
				clip(alpha - _Cutoff);
...
#define _Color_arr InstanceProperties
			...
			//	UNITY_SETUP_INSTANCE_ID(i);
				...
				float2  newUV = TRANSFORM_TEX(i.uv0, _MainTex);
				half alpha = tex2D(_MainTex, newUV).a * UNITY_ACCESS_INSTANCED_PROP(_Color_arr, _Color).a * i.color.a;
