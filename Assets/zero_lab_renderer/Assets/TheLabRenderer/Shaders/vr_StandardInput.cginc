...
{
    half3 albedo = UNITY_ACCESS_INSTANCED_PROP(_Color_arr, _Color).rgb * tex2D (_MainTex, texcoords.xy).rgb;
#if _DETAIL
    #if (SHADER_TARGET < 30)
...
half Alpha(float2 uv)
{
    ...
    return _Color.a;
#else
    return tex2D(_MainTex, uv).a * UNITY_ACCESS_INSTANCED_PROP(_Color_arr, _Color).a;
#endif
}
